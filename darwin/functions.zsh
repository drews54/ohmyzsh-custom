...
function to_hvc1 {
  ...
  elif [[ $encoder == vtb ]];  then ffmpeg_params='-hide_banner -i "%s" -c:v hevc_videotoolbox -q:v 75 -tag:v hvc1 -map_metadata 0 "%s"'
...
function lsrf {
  ...
  do
    ...
    then
      ...
      open -W ${app:+-a} ${app:+$app} $i && sleep 0.5
    else open ${app:+-a} ${app:+$app} $i && sleep 0.5
    fi
    if [[ -v interactive ]]
