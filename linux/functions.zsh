...
function to_hvc1 {
  ...
  local ffmpeg_params
  if   [[ $encoder == x265  ]]; then ffmpeg_params='-i "%s" -c:v libx265 -preset slow -tag:v hvc1 -map_metadata 0 "%s"'
  elif [[ $encoder == nvenc ]]; then ffmpeg_params='-hwaccel cuda -hwaccel_output_format cuda -i "%s" -c:v hevc_nvenc -preset p7 -b_ref_mode disabled -tag:v hvc1 -map_metadata 0 "%s"'
  ...
  do
    ...
    printf $ffmpeg_params $video ${video%.*}.hvc1.mp4 | xargs --open-tty ffmpeg
